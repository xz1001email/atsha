...
{
	SCL_out_low();//ÎªÁË±ÜÃâÒâÍâµÄÆô¶¯»òÍ£Ö¹Ìõ¼þ£¬Ò»¶¨ÒªÈ·±£SCLÊ±ÖÓÏßÎªµÍ¡£

	IIC_SDA(GPIO_OUT_HIGH);//ÏÈ¸Ä±äSDA ¹Ü½ÅµÄÊä³ö¼Ä´æÆ÷ODR£¬¸ßµçÆ½
	SDA_OUT();//ÉèÖÃSDA Êä³ö
	
	
	SCL_out_high();//ÉèÖÃSCL¹Ü½ÅÎª¸ßµçÆ½
	
	IIC_SDA(GPIO_OUT_LOW);//SDAÊä³öµÍµçÆ½
	SDA_OUT();//ÉèÖÃSDA Êä³ö

	
	SCL_out_low(); //ÉèÖÃSCL¹Ü½ÅÎªµÍµçÆ½£¬SCL²úÉúÏÂ½µÑØ
}
...
void software_IIC_stop(void)
{
	...
	IIC_SDA(GPIO_OUT_LOW);//ÏÈ¸Ä±äSDA ¹Ü½ÅµÄÊä³ö¼Ä´æÆ÷ODR£¬µÍµçÆ½
	SDA_OUT();//ÉèÖÃSDA Êä³ö

	SCL_out_high();	//ÉèÖÃSCL¹Ü½ÅÎª¸ßµçÆ½¡£SCL²úÉúÉÏ½µÑØ

	IIC_SDA(GPIO_OUT_HIGH);//SDAÊä³ö¸ßµçÆ½	
	SDA_OUT();//ÉèÖÃSDA Êä³ö	,SDA²úÉúÉÏ½µÑØ
...
bool software_IIC_read_ack()
{
	...
	while(READ_SDA()==NACK)
	{
		...
		{//max wait time = 16*period/4
			...
			//IIC_SDA(GPIO_OUT_HIGH);//SDA»Ö¸´³õÊ¼»¯×´Ì¬Êä³ö¸ßµçÆ½
			//SDA_OUT();//	
			return NACK;
		}
		error_times++;
